...
public class GuiContainerWorkbench extends GuiScreen {
	...
	public GuiContainerWorkbench(InventoryPlayer playerinventory, World w, int i, int j, int k,int wbb) {
    ...
    {
		if(selectedBlueprint < Main.craftFile.size() && canCraft) {
	        if (button.id == 0)
	        {
	...
	{
		if(selectedBlueprint > Main.craftFile.size() - 1) {
		...
		for(int m = 0; m < 3; m++)
		{
			for(int n = 0; n < 8; n++)
			{
				...
				if(blueprintNumber < Main.craftFile.size())
				{
						ItemStack is = Main.craftFile.get(blueprintNumber).getCraftedItem();
		...
		if(selectedBlueprint >= Main.craftFile.size())
		{
			return;
		}
		canCraft = true;
		...
		if(Main.craftFile.get(selectedBlueprint) != null)
		{
			CraftFileFirst cf = Main.craftFile.get(selectedBlueprint);
			...
			for(int r = 0; r < 3; r++)
			{
				for(int c = 0; c < 4; c++)
				{
					...
					if(recipeItemNumber < Main.recipe.get(cf).length)
					{
						//Get the itemstack required by the recipe
						ItemStack recipeStack = Main.recipe.get(cf)[recipeItemNumber];
	...
	protected void mouseClicked(int i, int j, int k)
	{
		...
		if (k == 0 || k == 1)
		{
			...
			{
				if(blueprintsScroll * 8 + 16 < Main.craftFile.size())
					blueprintsScroll++;
			}
			//Return if the selectedBlueprint is invalid
			if(selectedBlueprint >=  Main.craftFile.size())
			...
			{
				CraftFileFirst cf = Main.craftFile.get(selectedBlueprint);
				if(cf != null && recipeScroll * 4 + 12 <Main.recipe.get(cf).length)
					recipeScroll++;
			}
		}
	}
